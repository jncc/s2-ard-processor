...
if (__name__ == '__main__') and (mpiRank == 0):
    ...
    if len(sys.argv) == 1:
        ...
        if args.sensorlist:
            ...
            try:
                ...
                ### scisys temp fix - caveat: needs MPI ranks to equal number of granules being processed
                for ii in range(1, mpiSize):
                    # receive the ready status from worker
                    rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.READY, status=mpiStatus)
                    if DEBUG:
                        print("DEBUG-master PART 1 || send to rank:", ii, "time:", time.asctime(), "from node:",MPI.Get_processor_name(), "from rank", mpiRank, flush=True)
                    mpiComm.send([arcsiStages.ARCSIPART1, paramsLst[ii - 1]], dest=ii, tag=mpiTags.START)
                    # receive work done loop
                for ii in range(1, mpiSize):
                    # receive the ready status to unblock the worker
                    rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.DONE, status=mpiStatus)
                    if DEBUG:
                        print("DEBUG-master PART 1 || receive from", ii, "time:", time.asctime(), flush=True)
                    paramsLstTmp.append(rtnParamsObj)
                paramsLst = paramsLstTmp
                # force synchronise
                mpiComm.Barrier()
                ...
                if calc6SSREF:
                    ...
                    for ii in range(1, mpiSize):
                        ...
                        rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.READY, status=mpiStatus)
                        if DEBUG:
                            print("DEBUG-master PART 2 || send to rank:", ii, "time:", time.asctime(), "from node:", MPI.Get_processor_name(), "from rank", mpiRank, flush=True)
                        mpiComm.send([arcsiStages.ARCSIPART2, paramsLst[ii - 1]], dest=ii, tag=mpiTags.START)
                        # receive work done loop
                    for ii in range(1, mpiSize):
                        # receive the ready status to unblock the worker
                        rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.DONE, status=mpiStatus)
                        if DEBUG:
                            print("DEBUG-master PART 2 || receive from", ii, "time:", time.asctime(), flush=True)
                        paramsLstTmp.append(rtnParamsObj)
                    paramsLst = paramsLstTmp
                    # force synchronise
                    mpiComm.Barrier()
                ...
                if exportMetaData:
                    ...
                    for ii in range(1, mpiSize):
                        ...
                        rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.READY, status=mpiStatus)
                        if DEBUG:
                            print("DEBUG-master PART 3 || send to rank:", ii, "time:", time.asctime(), "from node:", MPI.Get_processor_name(), "from rank", mpiRank, flush=True)
                        mpiComm.send([arcsiStages.ARCSIPART3, paramsLst[ii - 1]], dest=ii, tag=mpiTags.START)
                        # receive work done loop
                    for ii in range(1, mpiSize):
                        # receive the ready status to unblock the worker
                        rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.DONE, status=mpiStatus)
                        if DEBUG:
                            print("DEBUG-master PART 3 || receive from", ii, "time:", time.asctime(), flush=True)
                        paramsLstTmp.append(rtnParamsObj)
                    paramsLst = paramsLstTmp
                    # force synchronise
                    mpiComm.Barrier()
                ...
                for ii in range(1, mpiSize):
                    ...
                    rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.READY, status=mpiStatus)
                    if DEBUG:
                        print("DEBUG-master PART 4 || send to rank:", ii, "time:", time.asctime(), "from node:", MPI.Get_processor_name(), "from rank", mpiRank, flush=True)
                    mpiComm.send([arcsiStages.ARCSIPART4, paramsLst[ii - 1]], dest=ii, tag=mpiTags.START)
                    # receive work done loop
                for ii in range(1, mpiSize):
                    # receive the ready status to unblock the worker
                    rtnParamsObj = mpiComm.recv(source=ii, tag=mpiTags.DONE, status=mpiStatus)
                    if DEBUG:
                        print("DEBUG-master PART 4 || receive from", ii, "time:", time.asctime(), flush=True)
                    paramsLstTmp.append(rtnParamsObj)
                paramsLst = paramsLstTmp
                # force synchronise
                mpiComm.Barrier()
    ...
    while True:
        ...
        # debug_folder = "/mnt/data/checkpoints_folder/" + checkpoint_folder

        if tag == mpiTags.START:
            # Do work!
            if tskData[0] == arcsiStages.ARCSIPART1:
                if DEBUG:
                    print("DEBUG-worker PART 1 START ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)
                paramsObj = arcsilib.arcsirun._runARCSIPart1(tskData[1])
                if DEBUG:
                    print("DEBUG-worker PART 1 END ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)

                # # scisys debug
                # if DEBUG:
                #     open(debug_folder + "/{}_rank-{}_node-{}_PART1.checkpoint".format(datetime.datetime.utcnow().strftime("%Y-%m-%d-%H%M%S.%fZ"), mpiRank, MPI.Get_processor_name()),"w").close()
                # # scisys debug
            elif tskData[0] == arcsiStages.ARCSIPART2:
                if DEBUG:
                    print("DEBUG-worker PART 2 START ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)
                paramsObj = arcsilib.arcsirun._runARCSIPart2(tskData[1])
                if DEBUG:
                    print("DEBUG-worker PART 2 END ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)

                # # scisys debug
                # if DEBUG:
                #     open(debug_folder + "/{}_rank-{}_node-{}_PART2.checkpoint".format(datetime.datetime.utcnow().strftime("%Y-%m-%d-%H%M%S.%fZ"), mpiRank, MPI.Get_processor_name()), "w").close()
                # # scisys debug
            elif tskData[0] == arcsiStages.ARCSIPART3:
                if DEBUG:
                    print("DEBUG-worker PART 3 START ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)
                paramsObj = arcsilib.arcsirun._runARCSIPart3(tskData[1])
                if DEBUG:
                    print("DEBUG-worker PART 3 END ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)

                # # scisys debug
                # if DEBUG:
                #     open(debug_folder + "/{}_rank-{}_node-{}_PART3.checkpoint".format(datetime.datetime.utcnow().strftime("%Y-%m-%d-%H%M%S.%fZ"), mpiRank, MPI.Get_processor_name()), "w").close()
                # # scisys debug
            elif tskData[0] == arcsiStages.ARCSIPART4:
                if DEBUG:
                    print("DEBUG-worker PART 4 START ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)
                paramsObj = arcsilib.arcsirun._runARCSIPart4(tskData[1])
                if DEBUG:
                    print("DEBUG-worker PART 4 END ||", "time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)

                # # scisys debug
                # if DEBUG:
                #     open(debug_folder + "/{}_rank-{}_node-{}_PART4.checkpoint".format(datetime.datetime.utcnow().strftime("%Y-%m-%d-%H%M%S.%fZ"), mpiRank, MPI.Get_processor_name()), "w").close()
                # # scisys debug
            else:
                raise ARCSIException("Don't recognise processing stage")
            if DEBUG:
                print("DEBUG-worker arcsiPART", int(tskData[0]) + 1, "send to master || time:", time.asctime(), "node:", MPI.Get_processor_name(), " my rank:", mpiRank, flush=True)
